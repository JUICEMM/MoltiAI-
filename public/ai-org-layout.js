(() => {
  ...
  const models = {
    chatgpt: {name:'ChatGPT', role:'AI Operations Manager', desc:'拆解任務、Routing、整合、QA 與 Human Approval。', keywords:['管理','任務','整合','流程','workflow','派工']},
    gemini: {name:'Gemini', role:'Research / Google / Data', desc:'搜尋、資料蒐集、競品、趨勢與多模態研究。', keywords:['研究','搜尋','資料','市場','競品','趨勢','google','數據','新聞','調查','比較']},
    claude: {name:'Claude', role:'Consultant / Strategy / Writing', desc:'策略、提案、制度文件、長文與顧問分析。', keywords:['策略','顧問','提案','報告','制度','政策','企劃','文案','課程','分析','治理','roadmap']},
    copilot: {name:'Copilot / Office Agent', role:'Microsoft 365 / Office-style', desc:'Word、Excel、PowerPoint、Outlook 類工作；Guest Mode 不存取私人 Microsoft 365 資料。', keywords:['excel','word','powerpoint','ppt','outlook','office','表格','試算表','信件','會議','文件','microsoft']},
    codex: {name:'Codex', role:'Code / GitHub / Engineering', desc:'程式、修復、測試、GitHub、API 與部署。', keywords:['程式','網站','github','git','api','修復','bug','部署','vercel','code','開發','測試','工程','資料庫']},
  };
  ...
  const buildPanel = () => {
    ...
    panel.innerHTML = `<div class="m-os-eyebrow">MOLTIAI AI OPERATING SYSTEM</div><h2>Michael 決策，ChatGPT 管理，專業 AI 執行</h2><p>Human Decision → ChatGPT Operations Manager → Gemini / Claude / Copilot / Codex。高風險動作保留 Human Approval。</p><div class="m-os-flow"><div class="m-os-node"><span>CEO / DECISION MAKER</span><strong>Michael</strong><p>Strategy · Budget · Final Approval</p></div><div class="m-os-arrow">↓</div><div class="m-os-node"><span>AI OPERATIONS MANAGER</span><strong>ChatGPT</strong><p>Agent · Workflow · Routing · QA · Execution Management</p></div><div class="m-os-arrow">↓</div><div class="m-os-grid"><div class="m-os-node"><span>RESEARCH</span><strong>Gemini</strong><b>Research / Google / Data</b><p>搜尋、資料蒐集、競品與趨勢研究。</p></div><div class="m-os-node"><span>CONSULTANT</span><strong>Claude</strong><b>Consultant / Strategy / Writing</b><p>策略、提案、制度文件與顧問分析。</p></div><div class="m-os-node"><span>OFFICE</span><strong>Copilot</strong><b>Microsoft 365 / Office</b><p>Guest Mode 做 Office 任務；私人 M365 資料仍需 Microsoft 登入。</p></div><div class="m-os-node"><span>ENGINEER</span><strong>Codex</strong><b>Code / GitHub / Engineering</b><p>程式、修復、測試、GitHub 與部署。</p></div></div></div><div class="m-os-router"><h3>ChatGPT AI Task Router</h3><p>輸入任務，先判斷應由哪個專業 AI 執行，再顯示 Workflow 與 Human Approval。</p><div class="m-os-row"><textarea placeholder="例如：研究 20 家上市櫃製造業潛在客戶，整理 AI 導入需求並提出開發策略。"></textarea><button>分析並路由</button></div><div class="m-os-result"></div></div><div class="m-os-tools"><button class="m-os-tool" data-nav="影片分析"><strong>影片分析 / PDF 報告</strong><span>Analyze URL → 競品 → Hook → 分鏡 → PDF</span></button><button class="m-os-tool" data-nav="腳本"><strong>腳本 / 15 秒影片</strong><span>Script + 圖片 → 15 秒短影音</span></button><button class="m-os-tool" data-nav="Video Factory"><strong>Video Factory</strong><span>保留完整內容與 AI 影音流程</span></button></div>`;
    const ta=panel.querySelector('textarea'), result=panel.querySelector('.m-os-result');
    panel.querySelector('.m-os-row button').addEventListener('click',()=>{const task=ta.value.trim();if(!task)return;const r=routeTask(task),p=models[r.primary],s=r.secondary?models[r.secondary]:null;result.textContent=`ChatGPT Routing\nPrimary: ${p.name} — ${p.role}${s?`\nCollaborator: ${s.name} — ${s.role}`:''}\nHuman Approval: ${r.approval?'Required':'Final review'}\n\nWorkflow\n1. Michael 定義目標與限制\n2. ChatGPT 拆解任務與成功標準\n3. ${p.name}${s?` + ${s.name}`:''} 執行\n4. ChatGPT 整合與 QA\n5. Michael 最終確認`;result.classList.add('show');});
  ...
  const buildGuestWorkbench = () => {
    ...
    box.innerHTML=`<div class="m-os-eyebrow">GUEST MULTI-MODEL WORKBENCH <span class="m-os-badge">免註冊</span></div><h2>ChatGPT → Gemini / Claude / Copilot / Codex</h2><p>訪客不需要建立 MoltiAI 帳號。模型憑證只放在伺服器端；先選 AI 專家、輸入任務，直接執行。</p><div class="m-guest-models">${Object.entries(models).map(([id,m])=>`<button class="m-guest-model ${id==='chatgpt'?'selected':''}" data-provider="${id}"><i class="m-guest-dot" data-dot="${id}"></i><strong>${m.name}</strong><span>${m.role}</span><small>${m.desc}</small></button>`).join('')}</div><div class="m-guest-grid"><textarea class="m-guest-textarea" placeholder="例如：幫我研究這家公司，整理 AI 導入機會、風險、3 個 Pilot 與下一步。"></textarea><button class="m-guest-run">直接執行</button></div><div class="m-guest-status">正在確認可用模型…</div><div class="m-guest-output"></div><div class="m-guest-note">Copilot 說明：真正的 Microsoft 365 Copilot 私有郵件、文件與企業資料存取需要 Microsoft Entra / 使用者授權；免登入 Guest Mode 只提供 Office 工作能力，不會假裝已讀取你的 Microsoft 365 私有資料。</div>`;
    ...
    const refresh=async()=>{try{const r=await fetch('/api/agents/run',{cache:'no-store'});if(!r.ok)throw new Error();const d=await r.json();Object.entries(d.providers||{}).forEach(([id,on])=>{const dot=box.querySelector(`[data-dot="${id}"]`);if(dot&&on)dot.classList.add('on');});const available=Object.entries(d.providers||{}).filter(([,on])=>on).map(([id])=>models[id]?.name||id);status.textContent=available.length?`伺服器可用：${available.join('、')}。訪客免註冊。`:'目前未偵測到外部模型憑證；仍可使用本地 Workflow Demo。';}catch{status.textContent='無法讀取模型狀態；仍可嘗試執行。';}};
    run.addEventListener('click',async()=>{const prompt=textarea.value.trim();if(!prompt)return;run.disabled=true;run.textContent='執行中…';output.classList.add('show');output.textContent=`正在交給 ${models[selected].name}…`;try{const r=await fetch('/api/agents/run',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({agentId:'strategy',agentName:models[selected].name,prompt,provider:selected})});const d=await r.json();output.textContent=`實際執行：${d.provider||'unknown'}${d.warning?`\n狀態：${d.warning}`:''}\n\n${d.output||'沒有回傳內容'}`;status.textContent=`Requested: ${models[selected].name}｜Actual: ${d.provider||'unknown'}｜Guest Mode`; }catch(e){output.textContent='執行失敗，請稍後再試。';status.textContent='API 暫時無法連線。';}finally{run.disabled=false;run.textContent='直接執行';}});
    refresh(); return box;
  };

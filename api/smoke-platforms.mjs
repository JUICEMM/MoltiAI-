...
const runOne=async(origin,item)=>{try{const r=await timeoutFetch(`${origin}/api/video/analyze`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(item)},60000);const text=await r.text();let j={};try{j=JSON.parse(text)}catch{};return{name:item.name,ok:r.ok&&Array.isArray(j.hooks)&&j.hooks.length>=3&&Array.isArray(j.storyboard)&&j.storyboard.length>=3,status:r.status,platform:j.platformLabel,confidence:j.confidence,hook:j.hooks?.[0]||'',metadataPlan:j.metadataPlan||'',error:j.error||j.detail||''}}catch(error){return{name:item.name,ok:false,error:error instanceof Error?error.message:String(error)}}};
export default async function handler(req,res){
  ...
  const results=await Promise.all(cases.map((item)=>runOne(origin,item)));

...
const timeoutFetch=async(url,options={},ms=60000)=>{const c=new AbortController();const t=setTimeout(()=>c.abort(),ms);try{return await fetch(url,{...options,signal:c.signal})}finally{clearTimeout(t)}};
export default async function handler(req,res){
  ...
  for(const item of cases){
    try{
      const r=await timeoutFetch(`${origin}/api/video/analyze`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(item)},60000);
      const text=await r.text();let j={};try{j=JSON.parse(text)}catch{}
      results.push({name:item.name,ok:r.ok&&Array.isArray(j.hooks)&&j.hooks.length>=3&&Array.isArray(j.storyboard)&&j.storyboard.length>=3,status:r.status,platform:j.platformLabel,confidence:j.confidence,hook:j.hooks?.[0]||'',metadataPlan:j.metadataPlan||'',error:j.error||j.detail||''});
    }catch(error){results.push({name:item.name,ok:false,error:error instanceof Error?error.message:String(error)});}
  }
